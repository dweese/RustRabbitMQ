        ...
        Fut: std::future::Future<Output = Result<InventoryResponse, RpcError>> + Send + 'static,
    {
        ...
        while let Some(delivery_result) = consumer_stream.next().await {
            match delivery_result {
                ...
                Err(e) => {
                    error!("CRITICAL: Fatal RabbitMQ error: {}", e);

                    // Basic cleanup
                    if let Err(cleanup_err) =
                        channel.close(0, "Shutting down due to fatal error").await
                    {
                        error!("Failed to close channel gracefully: {}", cleanup_err);
                    }

                    // Just exit - let the system orchestration handle restart
                    std::process::exit(1);
                }
            }
        }
