    ...
    impl RpcClient {
        ...
        fn get_channel(&self) -> Result<&Channel, RpcError> {
            self.channel.as_ref().ok_or_else(|| RpcError::ChannelError("No channel available".to_string()))
        }
        ...
        pub async fn call<T: Serialize, R: for<'de> Deserialize<'de>>(
            ...
            self.response_channels.lock().unwrap().insert(correlation_id.clone(), tx);
            ...
            ).await.map_err(|e| RpcError::PublishError(e.to_string()))?;
